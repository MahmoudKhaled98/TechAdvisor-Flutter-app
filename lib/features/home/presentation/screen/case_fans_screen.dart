...
class CaseFansScreen extends StatelessWidget {
  const CaseFansScreen({super.key});
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (casFans) => ListView.builder(
                  itemCount: casFans!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                        state1.selectedIndices.contains(casFans[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: casFans[index].name,
                        objectImageUrl: casFans[index]!.imageUrl,
                        objectSpecs: casFans[index]!.manufacturer,
                        objectPrice: casFans[index]!.price,
                        favBtn: FavBtn(
                            ...
                            isSelected
                                ...
                                : context.read<FavoriteBloc>().add(
                                    FavoriteEvent.addFavorite(FavoriteEntity(
                                        name: casFans[index].name,
                                        id: casFans[index].id,
                                        manufacturer:
                                            casFans[index].manufacturer,
                                        price: casFans[index].price,
                                        imageUrl: casFans[index].imageUrl)));
                          },
                        ),
