...
class ExternalStorageScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('External Storage', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (externalStorages) => ListView.builder(
                  itemCount: externalStorages!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(externalStorages[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: externalStorages[index].name,
                        objectImageUrl: externalStorages[index]!.imageUrl,
                        objectSpecs: externalStorages[index]!.manufacturer,
                        objectPrice: externalStorages[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          isSelected
                          //Remove item from favorites
                              ? context.read<FavoriteBloc>().add(
                              FavoriteEvent.removeFavorite(
                                  externalStorages[index]!.id))
                              : context.read<FavoriteBloc>().add(
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  name: externalStorages[index].name,
                                  id: externalStorages[index].id,
                                  manufacturer:
                                  externalStorages[index].manufacturer,
                                  price: externalStorages[index].price,
                                  imageUrl:
                                  externalStorages[index].imageUrl)));
                        },),
