...
class FanControllersScreen extends StatelessWidget {
  const FanControllersScreen({super.key});
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('Fan Controllers', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (fanControllers) => ListView.builder(
                  itemCount: fanControllers!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(fanControllers[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: fanControllers[index].name,
                        objectImageUrl: fanControllers[index]!.imageUrl,
                        objectSpecs: fanControllers[index]!.manufacturer,
                        objectPrice: fanControllers[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          isSelected
                          //Remove item from favorites
                              ? context.read<FavoriteBloc>().add(
                              FavoriteEvent.removeFavorite(
                                  fanControllers[index]!.id))
                              : context.read<FavoriteBloc>().add(
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  name: fanControllers[index].name,
                                  id: fanControllers[index].id,
                                  manufacturer:
                                  fanControllers[index].manufacturer,
                                  price: fanControllers[index].price,
                                  imageUrl:
                                  fanControllers[index].imageUrl)));
                        },),
