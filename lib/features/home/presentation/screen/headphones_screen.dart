    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('Headphones', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (headphones) => ListView.builder(
                  itemCount: headphones!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(headphones[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: headphones[index].name,
                        objectImageUrl: headphones[index]!.imageUrl,
                        objectSpecs: headphones[index]!.manufacturer,
                        objectPrice: headphones[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          ...
                          //Remove item from favorites
                              ...
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  ...
                                  id: headphones[index].id,
                                  manufacturer:
                                  headphones[index].manufacturer,
                                  price: headphones[index].price,
                                  imageUrl:
                                  headphones[index].imageUrl)));
                        },),
