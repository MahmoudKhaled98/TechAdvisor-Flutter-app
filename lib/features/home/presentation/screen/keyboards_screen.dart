    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('Keyboards', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (keyboards) => ListView.builder(
                  itemCount: keyboards!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(keyboards[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: keyboards[index].name,
                        objectImageUrl: keyboards[index]!.imageUrl,
                        objectSpecs: keyboards[index]!.manufacturer,
                        objectPrice: keyboards[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          ...
                          //Remove item from favorites
                              ...
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  name: keyboards[index].name,
                                  id: keyboards[index].id,
                                  manufacturer:
                                  keyboards[index].manufacturer,
                                  price: keyboards[index].price,
                                  imageUrl:
                                  keyboards[index].imageUrl)));
                        },),
