...
class MousePadsScreen extends StatelessWidget {
  const MousePadsScreen({super.key});
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('Mouse Pads', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (mousePads) => ListView.builder(
                  itemCount: mousePads!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(mousePads[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: mousePads[index].name,
                        objectImageUrl: mousePads[index]!.imageUrl,
                        objectSpecs: mousePads[index]!.manufacturer,
                        objectPrice: mousePads[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          ...
                          //Remove item from favorites
                              ...
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  name: mousePads[index].name,
                                  id: mousePads[index].id,
                                  manufacturer:
                                  mousePads[index].manufacturer,
                                  price: mousePads[index].price,
                                  imageUrl:
                                  mousePads[index].imageUrl)));
                        },),
