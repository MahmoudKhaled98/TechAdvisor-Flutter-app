...
class OperatingSystemsScreen extends StatelessWidget {
  const OperatingSystemsScreen({super.key});
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('operating Systems', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (operatingSystems) => ListView.builder(
                  itemCount: operatingSystems!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(operatingSystems[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: operatingSystems[index].name,
                        objectImageUrl: operatingSystems[index]!.imageUrl,
                        objectSpecs: operatingSystems[index]!.manufacturer,
                        objectPrice: operatingSystems[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          isSelected
                          //Remove item from favorites
                              ? context.read<FavoriteBloc>().add(
                              FavoriteEvent.removeFavorite(
                                  operatingSystems[index]!.id))
                              : context.read<FavoriteBloc>().add(
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  name: operatingSystems[index].name,
                                  id: operatingSystems[index].id,
                                  manufacturer:
                                  operatingSystems[index].manufacturer,
                                  price: operatingSystems[index].price,
                                  imageUrl:
                                  operatingSystems[index].imageUrl)));
                        },),
