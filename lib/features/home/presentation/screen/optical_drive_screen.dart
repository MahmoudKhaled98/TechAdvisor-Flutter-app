...
class OpticalDriveScreen extends StatelessWidget {
  const OpticalDriveScreen({super.key});
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('Optical Drives', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (opticalDrives) => ListView.builder(
                  itemCount: opticalDrives!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(opticalDrives[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: opticalDrives[index].name,
                        objectImageUrl: opticalDrives[index]!.imageUrl,
                        objectSpecs: opticalDrives[index]!.manufacturer,
                        objectPrice: opticalDrives[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          isSelected
                          //Remove item from favorites
                              ? context.read<FavoriteBloc>().add(
                              FavoriteEvent.removeFavorite(
                                  opticalDrives[index]!.id))
                              : context.read<FavoriteBloc>().add(
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  name: opticalDrives[index].name,
                                  id: opticalDrives[index].id,
                                  manufacturer:
                                  opticalDrives[index].manufacturer,
                                  price: opticalDrives[index].price,
                                  imageUrl:
                                  opticalDrives[index].imageUrl)));
                        },),
