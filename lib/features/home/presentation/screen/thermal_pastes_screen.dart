...
class ThermalPastesScreen extends StatelessWidget {
  const ThermalPastesScreen({super.key});
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Padding(
          ...
          child: Text('Thermal Paste', style: TextStyle(fontFamily: 'Schyler',
          ...
          return BlocBuilder<FirebaseDataSourceBloc, FirebaseDataSourceState>(
            builder: (context, state) {
              return state.when(
                ...
                loaded: (thermalPastes) => ListView.builder(
                  itemCount: thermalPastes!.length,
                  itemBuilder: (context, index) {
                    bloc.getFavoritesIds();
                    final isSelected =
                    state1.selectedIndices.contains(thermalPastes[index].id);
                    final path = isSelected
                        ? 'assets/svg_icons/starSelected.svg'
                        : 'assets/svg_icons/starUnselected.svg';
                    return ItemCard(
                        objectName: thermalPastes[index].name,
                        objectImageUrl: thermalPastes[index]!.imageUrl,
                        objectSpecs: thermalPastes[index]!.manufacturer,
                        objectPrice: thermalPastes[index]!.price,
                        favBtn: FavBtn(isSelected: isSelected, path: path, onTap: () {
                          isSelected
                          //Remove item from favorites
                              ? context.read<FavoriteBloc>().add(
                              FavoriteEvent.removeFavorite(
                                  thermalPastes[index]!.id))
                              : context.read<FavoriteBloc>().add(
                              FavoriteEvent.addFavorite(FavoriteEntity(
                                  name: thermalPastes[index].name,
                                  id: thermalPastes[index].id,
                                  manufacturer:
                                  thermalPastes[index].manufacturer,
                                  price: thermalPastes[index].price,
                                  imageUrl:
                                  thermalPastes[index].imageUrl)));
                        },),
